...
int main()
{
    ...
    // Error #3: you can't have an else without an if
    // also known as dangling-else problem
    // See the example on lines 20-27

    
    // Error #4: don't put the condition again after else (nor the inverse of the condition)
    if (cheerios < miniWheats) 
    {
        cout << "Go Mini Wheats!" << endl;
    }
    ...
    // Error #6: inadvertently using statements that do not evaluate to true or false as conditions
    // if (cheerios)
    // if (5)
    // if ("yes")
    // if (cheerios/5) 
    ...
    // Error #7: don't forget to use the {} for each if or else block, even if there is only one statement in each block
